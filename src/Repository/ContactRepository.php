...
class ContactRepository extends ServiceEntityRepository implements ImportableRepositoryInferface {
    ...
    public function findAllForSearch(): array {
        return $this->getEntityManager()
                        ->createQuery(
                                'SELECT c, UPPER(c.nom) canonized_nom, UPPER(c.prenom) canonized_prenom FROM App\Contact\Entity\Contact c order by c.nom ASC, c.prenom ASC'
                        )
                        ->getScalarResult();
    }
    ...
    protected function findContactByEmail(string $email): ?Contact {
        return $this->getEntityManager()
                        ->createQuery("SELECT c FROM App\Contact\Entity\ContactEmail e JOIN e.contact c WHERE e.email = :email")
    ...
    protected function findContactByPhone(string $phone): ?Contact {
        return $this->getEntityManager()
                        ->createQuery("SELECT c FROM App\Contact\Entity\ContactTelephone p JOIN p.contact c WHERE p.numero = :phone")
